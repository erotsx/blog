  ...
  methods: {
    infiniteHandler($state) {
      ...
      if (this.params.tagId === undefined) {
        getCategoryInfo(this.params.categoryId).then(data => {
          this.name = data.data.categoryName
          document.title = this.title + " - " + this.name;
        })
      } else {
        getTagInfo(this.params.tagId).then(data => {
          this.name = data.data.tagName
          document.title = this.title + " - " + this.name;
        })
      }
      searchArticle(this.params).then(data => {
        ...
        if (data.data.items.length) {
          this.page++;
          this.articleList.push(...data.data.items);
          $state.loaded();
        } else {
          $state.complete();
        }
