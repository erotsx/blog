      ...
      <div class="article-info-container">
        ...
        <div class="article-info">
          ...
          <div class="third-line">
            ...
            <!-- 阅读量 -->
            <span>
              <i class="iconfont iconliulan"/> 阅读量: {{ article.viewCounts }}
            </span>
            <span class="separator">|</span>
            <!-- 评论量 -->
            <span>
              <i class="iconfont iconpinglunzu1"/>评论数: {{ article.commentCounts }}
            </span>
          </div>
        </div>
      </div>
            ...
            <div class="article-list">
              ...
              >
                ...
                <div class="content">
                  ...
                  <div class="content-time">{{ item.createDate | date }}</div>
                </div>
              </div>
            </div>
          </v-card>
...
export default {
  ...
  methods: {
    getArticle() {
      const that = this;
      getArticle(this.$route.path).then(data => {
        document.title = data.data.title;

        this.markdownToHtml(data.data);

        this.$nextTick(() => {
          this.wordNum = data.data.wordCounts;
          this.readTime = Math.round(this.wordNum / 160) + "分钟";
    ...
    markdownToHtml(article) {
      ...
      // 将markdown替换为html标签
      article.articleContent = md.render(article.body.content);
  ...
  computed: {
    ...
    articleCover() {
      return (
        "background: url(" +
        "https://www.notion.so/image/https%3A%2F%2Fwww.notion.so%2Fimages%2Fpage-cover%2Fgradients_11.jpg?table=collection&id=d0dcb5f1-9324-4c20-9a6f-2446aac9b4f9&spaceId=2f6f3d8b-da16-4d45-be45-4e1ecc295f74&width=2000&userId=2711b3bd-8fe1-40f8-b972-6c2370eb1135&cache=v2"
        +
