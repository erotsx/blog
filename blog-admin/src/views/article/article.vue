<template>
  <el-card class="main-card">
    ...
    <div class="article-title-container">
      ...
      />
      <el-button
        v-if="article.id == null || article.status === '草稿'"
        type="danger"
        size="medium"
        class="save-btn"
    ...
    <el-dialog :visible.sync="addOrEdit" width="40%" top="3vh">
      ...
      <el-form label-width="80px" size="medium" :model="article">
        ...
        <el-form-item label="上传封面">
          ...
          >
            <i v-if="article.cover === ''" class="el-icon-upload" />
            <div v-if="article.cover === ''" class="el-upload__text">
...
export default {
  ...
  computed: {
    tagClass() {
      return function(item) {
        if (this.containTag(item)) { return 'tag-item-select' }
        return 'tag-item'
      }
    }
  },
  ...
  methods: {
    ...
    openModel() {
      ...
      if (this.article.body.content.trim() === '') {
    ...
    autoSaveArticle() {
      ...
      if (
        ...
        this.article.body.content.trim() !== '' &&
    ...
    addTag(item) {
      if (!this.containTag(item)) {
        this.article.tags.push(item)
      }
    },
    removeTag(item) {
      this.article.tags = this.article.tags.filter(i => { return i.tagName !== item.tagName })
    },
    containTag(item) {
      let i
      for (i of this.article.tags) {
        if (i.tagName === item.tagName) return true
      }
      return false
    }
  }
